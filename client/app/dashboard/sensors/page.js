        ...
        <div className="ml-64 p-10 min-h-screen bg-background transition-colors duration-300">
            <div className="max-w-[1600px] mx-auto">
                <div className="flex justify-between items-end mb-10">
                    <div>
                        <h1 className="text-4xl font-black text-foreground tracking-tighter mb-2 flex items-center gap-4">
                            IOT Sensor Network
                        ...
                        <p className="text-foreground/50 font-medium">Manage and monitor your precision agriculture sensor array.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {sensors.map((sensor) => (
                        <div key={sensor.id} className="bg-card rounded-[2.5rem] p-8 border border-border relative group hover:bg-foreground/[0.02] transition-all">
                            ...
                            <div className="mb-8">
                                <span className="text-[10px] font-bold text-foreground/40 uppercase tracking-widest">{sensor.id} • {sensor.type}</span>
                                <h3 className="text-xl font-bold text-foreground mt-1">{sensor.name}</h3>
                            </div>

                            <div className="flex items-end justify-between">
                                <div>
                                    <div className="text-3xl font-black text-foreground tracking-tighter">{sensor.value}</div>
                                    <div className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${sensor.status === 'online' ? 'text-accent' : 'text-red-500'}`}>
                                        {sensor.status}
                                    </div>
                                </div>
                                <button className="w-10 h-10 bg-foreground/5 rounded-xl flex items-center justify-center text-foreground hover:bg-accent hover:text-background transition-all">
                                    <Activity size={18} />
                                </button>
                            </div>
                        </div>
