            ...
            <div className="max-w-[1600px] mx-auto">
                <div className="flex justify-between items-end mb-10">
                    <div>
                        ...
                        <p className="text-gray-500 font-medium">Schedule and manage daily agricultural operations and labor.</p>
                    </div>
                        ...
                        <div className="bg-card p-8 rounded-[2.5rem] border border-white/5">
                            <div className="flex justify-between items-center mb-8">
                                ...
                                <div className="flex gap-2">
                                    <button className="p-2 bg-white/5 rounded-lg text-gray-400 hover:text-white transition-colors">{'<'}</button>
                                    <button className="p-2 bg-white/5 rounded-lg text-gray-400 hover:text-white transition-colors">{'>'}</button>
                                </div>
                            </div>
                            <div className="grid grid-cols-7 gap-2 mb-4">
                                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                                    <span key={`${d}-${i}`} className="text-[10px] font-black text-gray-600 text-center uppercase">{d}</span>
                                ))}
                            </div>
                            <div className="grid grid-cols-7 gap-2">
                                {Array.from({ length: 31 }).map((_, i) => (
                                    <button
                                        key={i}
                                        className={`h-10 rounded-xl text-xs font-bold transition-all ${i + 1 === 26 ? 'bg-accent text-[#020603]' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}
                                    >
                                        {i + 1}
                                    </button>
                                ))}
                        ...
                        <div className="bg-card rounded-[2.5rem] border border-white/5 overflow-hidden">
                            <div className="p-8 border-b border-white/5 flex items-center justify-between">
                                ...
                                <div className="flex bg-white/5 p-1 rounded-xl">
                                    <button className="px-4 py-2 bg-accent text-[#020603] rounded-lg text-xs font-bold uppercase tracking-widest">List</button>
                                    <button className="px-4 py-2 text-gray-500 rounded-lg text-xs font-bold uppercase tracking-widest hover:text-white transition-colors">Board</button>
                                </div>
                            </div>
                            <div className="p-8 space-y-8 relative">
                                <div className="absolute left-[59px] top-8 bottom-8 w-px bg-white/5"></div>
                                {events.map((event) => (
                                    <div key={event.id} className="flex gap-10 items-start relative z-10">
                                        <div className="w-16 shrink-0 text-right">
                                            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{event.time}</span>
                                        </div>
                                        <div className="p-6 bg-white/[0.02] border border-white/5 rounded-[2rem] flex-1 group hover:bg-white/[0.05] hover:border-accent/20 transition-all">
                                            <div className="flex justify-between items-start mb-4">
                                                <div>
                                                    <h4 className="text-lg font-bold text-white tracking-tight">{event.title}</h4>
                                                    <div className="flex items-center gap-4 mt-1 text-gray-500 text-xs font-medium">
